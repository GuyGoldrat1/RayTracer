...
class Camera:
    def __init__(self, position, look_at, up_vector, screen_distance, screen_width):
        ...
        self.Vz = self.Normalize(self.look_at - self.position)  # Forward vector
        self.Vx = self.Normalize(np.cross(self.Vz, self.up_vector))  # Right vector
        self.Vy = np.cross(self.Vx, self.Vz)  # Up vector (not normalized)
